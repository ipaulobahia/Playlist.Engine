      ...
      <div className="grid grid-cols-1 xl:grid-cols-3">
        <div className="col-span-1 pb-5 pr-3 space-y-3">
          <div className="flex flex-col space-y-1.5">
            <span className="text-xl font-semibold leading-none tracking-tight">{t('Operator-Selection')}</span>
            <span className="text-sm text-muted-foreground">
              {t('Pesquise pelo o seu operador ou filtre pelos perfils disponiveis na sua emissora.')}
            </span>
          </div>
          <div className="flex flex-row items-center justify-between gap-x-2">
            <div className="relative w-full">
              <Input type="operador" className="dark:bg-black border-muted-foreground/25 placeholder:text-xs" placeholder={t('Placeholder-Selection-Operator')} />
        ...
        <div className="grid max-h-[420px] overflow-auto grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 col-span-2 gap-4">
          {operators.map((operator) => <DrawerDialogOperatorPin key={operator.id} operator={operator} />)}
        </div>
      </div>
    </Card>
  )
